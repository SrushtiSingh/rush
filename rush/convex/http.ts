...
Promise<WebhookEvent | undefined> => {
    ...
    const svixHeaders = {
        ...
        "svix-timestamp": req.headers.get
        ("svix-timestamp")!,
        "svix-signature": req.headers.get
        ("svix-signature")!,
    };
...
const handleClerkWebhook = httpAction(async (ctx,
req) => {
    ...
    case "user.updated":
       { console.log("Creating/Updating User:", event.
        data.id)

    await ctx.runMutation(internal.user.create, {
        username: `${event.data.first_name} ${event.
        data.last_name}`,
